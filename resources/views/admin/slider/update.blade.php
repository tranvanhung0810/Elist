...
<div class="container">
	<form action="{{ route('slider.update',['slider'=>$sliders->slider_id]) }}" method="POST" role="form">
    @csrf
    @method('PUT')
  <div class="form-row">
    <div class="form-group col-lg-7 @error('slider_name') has-error  @enderror">
        ...
        @if(Session::has('success'))
          ...
          @endif 
      <input type="text" class="form-control" name="slider_name" value="{{$sliders->slider_name}}"  placeholder="Nhập Tên Banner">
...
<a class="btn btn-primary" data-toggle="modal" href='#modal-file'><i class="glyphicon glyphicon-picture"></i></a>
     <input type="text" name="slider_image" id="image">
      ...
      @enderror
        ...
        <img src="{{url('uploads')}}/{{$sliders->slider_image}}" style="height: 100px;width: 100px;" id="img" alt="">
      </div>
    </div>
   ...
   <div class="form-group col-lg-7 @error('slider_link') has-error  @enderror">
      ...
      <input type="text" class="form-control" name="slider_link" value="{{$sliders->slider_link}}"  placeholder="Nhập Links Banner">
      @error('slider_link')
      <small class="help-block">{{$message}}</small>
      @enderror
    </div>
     <div class="form-group col-lg-7 @error('slider_title') has-error  @enderror">
      ...
      <input type="text" class="form-control" name="slider_title" value="{{$sliders->slider_title}}"  placeholder="Nhập Tiêu Đề">
      @error('slider_title')
      <small class="help-block">{{$message}}</small>
      @enderror
    </div>
     ...
     <div class="form-group col-lg-7 @error('slider_description') has-error  @enderror">
      ...
      <textarea name="slider_description" id="content" class="form-control">{{$sliders->slider_description}}</textarea>
